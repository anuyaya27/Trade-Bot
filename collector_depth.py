...
async def mainCollectorDepthTask():
    ...
    tasks = [asyncio.create_task(request_depth(sym, freq, limit)) 
             for sym in symbols]
    ...
    for fut in asyncio.as_completed(tasks, timeout=timeout):
        try:
            ...
            log.warning(f"Timeout {timeout} seconds when requesting order 
                        book data.")
        except Exception as e:
    ...
    log.info(f"<=== End depth collection task. {len(results)} responses stored. 
             {duration:.2f} seconds processing time.")
...
async def request_depth(symbol, freq, limit):
    ...
    depth = {}
    try:
        depth = App.client.get_order_book(symbol=symbol, limit=limit)
    ...
    depth['timestamp'] = pandas_get_interval(
        freq=freq, timestamp=requestTime)[0]
...
def start_collector_depth():
    ...
    App.client = Client(api_key=App.config["api_key"], 
    ...
    # Add job for collecting order book data (
    # only frequency field changes in the job creation)
    freq = App.config["collector"]["depth"]["freq"]
